...
vmexit_handle_cpuid(VIRTUAL_MACHINE_STATE * vcpu)
{
    ...
    //
    // pass-through to bare-metal CPUID — preserves subleaf semantics for
    // invalid/HV-synth leaves (the CPU itself returns the correct response
    // per Intel SDM Vol 2). avoids the cached-invalid-leaf path which
    // ignored subleaf and broke detection-tool checks that probe with
    // subleaf=0xFF (e.g. VMAware is_root_partition reads CPUID 0x40000003
    // with subleaf 0xFF -> bare metal returns zeros, but the cached value
    // would be the subleaf-0 response with EBX bit 0 = 1, false-positively
    // flagging us as a Hyper-V root partition).
    //
    __cpuidex(cpu_info, (int)leaf, (int)subleaf);

    //
    // leaf 1: clear the hypervisor present bit (ECX[31]) and SMX (ECX[6])
    // to stay consistent with our IA32_FEATURE_CONTROL masking. matches
    // a bare-metal "VMX BIOS-locked, no SMX" profile that is the most
    // common real-world stealth posture for detection tools.
    //
...
vmexit_handle_mov_cr(VIRTUAL_MACHINE_STATE * vcpu)
{
    ...
    case 0: // MOV to CR
    {
        ...
        case 4:
        {
            ...
            {
                //
                // bare-metal behavior when IA32_FEATURE_CONTROL has VMX disabled
                // (which we fake under stealth): writing CR4.VMXE=1 -> #GP(0).
                // mirror that so detection tools probing CR4 see consistent state.
                //
                if (desired & CR4_VMX_ENABLE_FLAG)
                {
                    vmexit_inject_gp();
                    vcpu->advance_rip = FALSE;
                    break;

...
typedef struct _VIRTUAL_MACHINE_STATE {
    ...
    BOOLEAN     launched;
    BOOLEAN     advance_rip;
    UINT64      launch_err;       // VM_INSTRUCTION_ERROR from failed VMLAUNCH
    UINT32      sipi_seen;        // count of SIPI vmexits handled
    UINT32      init_seen;        // count of INIT_SIGNAL vmexits handled
    UINT32      vmentry_fail_seen; // count of VMENTRY_FAILURE_GUEST_STATE vmexits
    UINT32      last_sipi_vec;    // last SIPI vector observed
    ...
    // Phase 7d: EPT cloak MTF single-step state. When EPT_VIOLATION (read)
    // hits a cloaked exec-only page, handler swaps PML1 to real_pfn R=1 X=0
    // and arms MTF; on MTF (reason 37) we swap back to alt_pfn X=1 R=0 so
    // the next instruction-fetch from RIP path still hits the patched copy.
    BOOLEAN          mtf_pending_restore;
    PVOID            mtf_pml1;       // PEPT_PML1_ENTRY (typed via void* to avoid ia32.h cycle)
    UINT64           mtf_alt_pfn;
...
#define VMCALL_INVEPT_ALL       0x00000003

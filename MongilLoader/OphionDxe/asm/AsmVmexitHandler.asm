; AsmVmexitHandler.asm - VMX-root vm-exit entry
; Ported from Ophion/asm/AsmVmexitHandler.asm.
; vmexit_handler / vmx_vmresume / vmx_return_rsp_for_vmxoff /
; vmx_return_rip_for_vmxoff defined in C (VmmStubs.c for now).
...
; HOST_RIP target — CPU jumps here on every vm-exit.
;
...
asm_vmexit_handler PROC
    ...
    mov     rdx, [rsp + 01A8h]      ; arg2: vcpu pointer

    sub     rsp, 020h
    call    vmexit_handler
    add     rsp, 020h

    ; Return value: TRUE = vmxoff, FALSE = vmresume.
    cmp     al, 1
    je      VmxoffPath
...
VmxoffPath PROC
    ...
    add     rsp, 0120h          ; xmm 0x110 + rflags 0x08 + padding 0x08
    ...
    xor     rax, rax            ; signal vmxoff success to vmcall caller

    ret                         ; return to guest code (after vmcall)
